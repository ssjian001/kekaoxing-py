...
# ══════════════════════════════════════════════════════════════════════
tasks = [
    (1, 1, "高温工作寿命测试(HTOL)", "环境试验", "GB/T 38046-2023 §5.2", 1, 1, json.dumps([2,3]), 168, 0, 100.0, "completed", 1, '{"temp":85,"humidity":50}', "", json.dumps([]), "85°C/50%RH 额定电压持续工作", "", "", "±10%输出电压偏差", 1, date_str(-80), date_str(-70), d(-85), d(-70)),
    (2, 1, "低温启动测试", "环境试验", "GB/T 38046-2023 §5.3", 1, 1, json.dumps([3]), 72, 3, 100.0, "completed", 2, '{"temp":-40}', "", json.dumps([1]), "-40°C存储后开机", "", "", "正常启动无故障", 2, date_str(-80), date_str(-70), d(-85), d(-70)),
    (3, 1, "温度循环测试(TC)", "环境试验", "GB/T 38046-2023 §5.4", 3, 1, json.dumps([2]), 240, 5, 65.0, "in_progress", 1, '{"temp_high":85,"temp_low":-40,"cycles":500}', "", json.dumps([1,2]), "500次温度循环(-40↔85°C)", "", "", "功能全程正常", 3, date_str(-60), "", d(-60), d(-30)),
    (4, 1, "随机振动测试", "机械试验", "GB/T 38046-2023 §5.5", 2, 2, json.dumps([3]), 8, 0, 100.0, "completed", 1, '{"psd":"0.01g²/Hz","freq":"10-2000Hz"}', "", json.dumps([]), "三轴向随机振动X/Y/Z各2h", "", "", "无机械损伤", 4, date_str(-75), date_str(-70), d(-80), d(-70)),
    (5, 1, "绝缘耐压测试", "其他", "GB/T 38046-2023 §5.6", 5, 5, json.dumps([1,2,3]), 1, 0, 100.0, "completed", 2, '{"voltage":2500}', "", json.dumps([]), "2500V AC 1min 漏电流<5mA", "", "", "漏电流<5mA", 5, date_str(-85), date_str(-85), d(-85), d(-85)),
    (6, 2, "VCU 高低温工作测试", "环境试验", "QC/T 1088-2023 §4.1", 1, 1, json.dumps([4,11]), 168, 0, 40.0, "in_progress", 1, '{"temp":85}', "", json.dumps([]), "高温85°C 额定电压168h", "", "", "功能正常", 6, date_str(-20), "", d(-30), d(-10)),
    (7, 2, "VCU 振动耐久测试", "机械试验", "QC/T 1088-2023 §4.2", 2, 2, json.dumps([4]), 24, 2, 10.0, "in_progress", 2, '{"psd":"0.02g²/Hz"}', "", json.dumps([6]), "24h 随机振动", "", "", "无损坏", 7, date_str(-15), "", d(-20), d(-10)),
    (8, 2, "VCU 盐雾测试", "环境试验", "QC/T 1088-2023 §4.3", 3, 6, json.dumps([11]), 96, 4, 30.0, "in_progress", 3, '{"concentration":"5%"}', "", json.dumps([]), "96h 中性盐雾", "", "", "腐蚀面积<5%", 8, date_str(-10), "", d(-12), d(-5)),
    (9, 4, "OBC 效率测试", "其他", "ISO 26262 §8", 4, 4, json.dumps([8,9,14,15]), 8, 0, 0.0, "pending", 2, '{"load":"0-100%"}', "", json.dumps([]), "全负载范围效率曲线", "", "", "满载效率>94%", 9, "", "", d(-5), d(-2)),
    (10, 4, "OBC 保护功能测试", "其他", "ISO 26262 §9", 5, 5, json.dumps([9]), 4, 1, 0.0, "pending", 1, '{"overvoltage":1}', "", json.dumps([9]), "过压/过流/过温保护", "", "", "保护动作正常", 10, "", "", d(-3), d(-2)),
    (11, 4, "OBC 温度循环测试", "环境试验", "ISO 26262 §10", 3, 1, json.dumps([14]), 120, 3, 0.0, "pending", 3, '{"cycles":200}', "", json.dumps([]), "200次温度循环", "", "", "无损坏", 11, "", "", d(-3), d(-2)),
    (12, 3, "MCU SiC 高温反偏测试(HTRB)", "环境试验", "IEC 60747-9", 1, 1, json.dumps([6]), 1000, 0, 50.0, "paused", 1, '{"vds":800,"temp":150}', "", json.dumps([]), "1000h 高温反偏", "", "", "漏电流<100μA", 12, date_str(-100), date_str(-60), d(-100), d(-60)),
]
for t in tasks:
