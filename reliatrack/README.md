...
| 索引 | Tab | 核心功能 |
|-----|-----|---------|
| 0 | 仪表盘 | 健康度卡片(评分0-100) + 左右两栏(4KPI+环形图 / 3KPI+独立进度环卡片) + Bug Tracker KPI(待处理/本周关闭/平均停留/aging警告) |
| 1 | 项目管理 | 项目 CRUD + 搜索 |
| 2 | 样品管理 | 样品池 + 出入库 + 批量导入 |
...
```
main.py              # 入口（MainWindow, 8个Tab, 快捷键分发）
src/
├── constants.py     # 全局常量
...
└── views/           # Qt 视图
    ...
    │   ├── quick_create.py    # 快速创建（C键弹窗）
    │   └── resolve_dialog.py  # 关闭流程（resolution强制）
    ├── widgets/               # 可复用组件
    │   ├── task_table.py      # 任务表格
    │   ├── gantt_widget.py    # 甘特图
...
E2E 测试需 offscreen 模式：`QT_QPA_PLATFORM=offscreen .venv/bin/python tests/test_e2e_full.py`。704 个 pytest 测试全通过。
