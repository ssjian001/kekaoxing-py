...
def render() -> None:
    ...
    with st.sidebar:
        ...
        with st.form("equipment_form", clear_on_submit=True):
            ...
            eq_cal = st.text_input(
                "校准日期 (YYYY-MM-DD)", max_chars=30,
                placeholder="如: 2026-06-01",
            )
            eq_next_cal = st.text_input(
                "下次校准日期", max_chars=30,
    ...
    with tab1:
        ...
        if equipments:
            ...
            if eq_map:
                ...
                with col_e1:
                    st.markdown("#### 编辑")
                    new_eq_name = st.text_input(
                        "名称", value=eq.name, max_chars=200, key="eq_edit_name",
                    )
                    new_eq_loc = st.text_input(
                        "位置", value=eq.location, max_chars=200, key="eq_edit_loc",
                    )
                    cur_status_label = EQUIPMENT_STATUS_LABELS.get(eq.status, "正常")
                    new_eq_status = st.selectbox(
                        "状态",
                        list(EQUIPMENT_STATUS_LABELS.values()),
                        index=(
                            list(EQUIPMENT_STATUS_LABELS.values()).index(cur_status_label)
                            if cur_status_label in EQUIPMENT_STATUS_LABELS.values()
                            else 0
                        ),
                        key="eq_edit_status",
                    )
                ...
                with col_e2:
                    if render_delete_confirm(f"设备 {eq.name}", "eq_del"):
                        try:
                            eq_svc.delete(eq.id)
                            st.success("已删除")
                            st.rerun()
    ...
    with tab2:
        ...
        if technicians:
            ...
            if tech_map:
                ...
                with col_t1:
                    ...
                    new_t_name = st.text_input(
                        "姓名", value=tech.name, max_chars=200, key="t_edit_name",
                    )
                    new_t_role = st.text_input(
                        "职位", value=tech.role, max_chars=200, key="t_edit_role",
                    )
                    new_t_dept = st.text_input(
                        "部门", value=tech.department, max_chars=200, key="t_edit_dept",
                    )
                    new_t_phone = st.text_input(
                        "电话", value=tech.phone, max_chars=200, key="t_edit_phone",
                    )
                    new_t_email = st.text_input(
                        "邮箱", value=tech.email, max_chars=200, key="t_edit_email",
                    )
                    if st.button("保存"):
                        tech_svc.update(
                            tech.id,
                ...
                with col_t2:
                    if render_delete_confirm(f"技术员 {tech.name}", "tech_del"):
                        try:
            ...
            st.info("暂无技术员。请在侧边栏「新增技术员」表单中添加。")
