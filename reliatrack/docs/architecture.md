...
| 索引 | Tab | 核心组件 |
|-----|-----|---------|
| 0 | 📊 仪表盘 | SaaS v2: Header + 测试进度堆叠条卡片(_StackedBar) + 左栏(3KPI+环形图) + 右栏(4KPI+进度环) + Bug KPI(待处理/本周关闭/平均停留/aging警告), QPainter |
| 1 | 📁 项目管理 | 项目 CRUD + 搜索过滤 |
...
| 4 | 🔧 设备管理 | 设备 CRUD + 校准管理 + 技术员管理（内部子 Tab） |
| 5 | 📚 知识库 | 失效模式 CRUD + 关键词搜索 |
| 6 | 🐛 Issue 管理 | 看板(4列拖拽+aging色块+closed折叠) / 列表(横筛选面板+FA/CAPA面板+批量操作) Tab切换 + 详情弹窗(评论+活动日志+FA+CAPA) + 快速创建 C键 + 关闭 resolution强制 |

> 注意：Tab 索引以 main.py 实际 addTab 顺序为准。原 Tab 4「Issue 追踪」已合并到 Bug 管理（现 Issue 管理），索引已左移。
