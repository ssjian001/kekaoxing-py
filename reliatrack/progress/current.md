...
2026-08-12 日历弹窗日期裁剪修复 + 总结报告弹窗提示 + 导出子菜单恢复 + 样品二维码真实化
        （远端 main = 2e011f2）。
2026-08-13 全面代码审计（166文件/39K行）无 P0/P1，修复 4 P2 + 1 附带资源泄漏
        （undo hasattr死代码/主题+命令面板静默except/schema硬编码断言/QMenu泄漏），
        703 tests 全过，远端 main = 9fa4477。

### 最近完成（2026-08-12 第二批）

- **样品二维码真实化 (2e011f2)**: 样品标签 QR 码从伪矩阵改为真实标准 QR (segno)。
  原 _generate_qr_matrix 只画 3 定位角+同步线+哈希随机点阵（装饰图, 扫码器无法识别）。
  segno (ISO/IEC 18004) 生成真实可扫描 QR, 编码 RELIATRACK-SAMPLE:SN=<sn>&ID=<id>。
  验证: 渲染像素重建矩阵与 segno 原始矩阵 100% 一致; 新增 test_sample_tag.py 10 测试。
- **导出子菜单恢复 (1d4403d)**: 恢复 2026-07-26 76f20c5 移除的通用导出入口
  (ExportDialog: 任务/Issue/样品/综合/DVP&R/8D, Ctrl+E)。用'导出'子菜单归拢
  通用导出 + 导出全景总结简报, 避免平铺混乱。新增 test_export_menu.py 10 测试。
...
- **就地编辑器 popup 竞态修复 (2026-08-11, cfa9759)**: 双击修改技术员无效 —
  focusOut(popup 关闭) 抢在 activated 前触发且无条件设防重入标记 → activated 提交被吃。
  _commit 全部返回 bool，仅真提交设标记；+3 回归测试，全量 687 passed。
  同类排查: 全项目其他交互路径(结果矩阵/拖拽/筛选 combo)无此模式；DQE Pro 常驻 combo 无竞态
- **UI 异常修复 (2026-08-10, 2be86be)**: result_matrix 摘要通过率颜色动态属性
  rate-class 缺 unpolish/polish → 数据刷新后颜色不更新；+ 无数据分支清旧属性
- **就地编辑器残留修复 (2026-08-10, a96c0f1)**: 双击状态/技术员/类别列点 popup 外部
  编辑器残留需再点一次 — PopupFocusReason 失焦仅当 popup 仍可见时跳过；
  +2 回归测试。offscreen 复现验证 A/B 两场景
- **UI 特性测试 (2026-08-10, 1bd9771)**: tests/test_ui_features.py 16 个测试 —
  6 Tab 布局/搜索过滤/设备 location 链路/排程技术员容量/remind_at roundtrip
